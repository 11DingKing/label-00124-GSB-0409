...
class BeamformingOutput:
    ...
    convergence_curve: Optional[np.ndarray] = None  # 收敛曲线(用于LMS)
...
class AdaptiveBeamformer:
    ...
    def _rls(
        ...
        # 初始化 RLS 参数
        # P(0) = δ^(-1) * I
        P = np.eye(self.num_elements, dtype=np.complex128) / delta
        ...
        # RLS 迭代
        for n in range(num_samples):
            x_n = array_signal[:, n:n+1]  # shape: (M, 1)
            ...
            # K(n) = P(n-1)*x(n) / (λ + x^H(n)*P(n-1)*x(n))
            x_H = x_n.conj().T
            denominator = forgetting_factor + (x_H @ P @ x_n).item()
            K_n = (P @ x_n) / denominator
            
            # P(n) = (P(n-1) - K(n)*x^H(n)*P(n-1)) / λ
            P = (P - K_n @ x_H @ P) / forgetting_factor
